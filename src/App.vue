...
const nextImage = () => {
  if(activeImageIndex.value < productImages.length){
    activeImageIndex.value++
  }
...
<template>
  <div class="pt-4 md:pt-20 grid gap-5 w-[90%] md:w-4/5 mx-auto relative">    
    <Navbar  />

    <div class="md:mt-16 flex flex-col sm:flex-row justify-center items-center sm:gap-8 md:gap-10 lg:gap-20">
      <div class="order-1">
        <div class="relative">
          <div @click="prevImage" class="cursor-pointer bg-white absolute top-1/2 left-2 w-10 h-10 flex justify-center items-center md:hidden rounded-full ">
            <img src="./assets/images/icon-previous.svg" alt="prev">
          </div>
          <img :src="productImages[activeImageIndex]" alt="image" class="w-80 h-[350px] md:rounded-lg object-cover">
          <div @click="nextImage" class="cursor-pointer bg-white absolute top-1/2 right-2 w-10 h-10 flex justify-center items-center md:hidden rounded-full ">
      ...
      <div class="order-2 mt-6 sm:mt-0 flex flex-col gap-4">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3 sm:gap-2">
          <div class="w-full sm:w-fit rounded-md py-2 px-2 flex justify-between items-center gap-4 bg-[hsl(223,64%,98%)]">
            <img src="./assets/images/icon-minus.svg" alt="minus" class="cursor-pointer">
            <p> {{ count }}</p>
            <img  src="./assets/images/icon-plus.svg" alt="plus" class="cursor-pointer">
          </div>
